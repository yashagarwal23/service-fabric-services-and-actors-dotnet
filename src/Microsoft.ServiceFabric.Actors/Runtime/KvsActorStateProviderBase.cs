...
namespace Microsoft.ServiceFabric.Actors.Runtime
{
        ...
        : IActorStateProvider, VolatileLogicalTimeManager.ISnapshotHandler, IActorStateProviderInternal, IInternalStatefulServiceReplica
    {
            ...
            try
            {
                ...
                await this.actorStateProviderHelper.ExecuteWithRetriesAsync(
                    async () =>
                    {
                        ...
                        this.InitializeAndStartLogicalTimeManager(cancellationToken);
                    },
        ...
        private async Task CancelStateProviderInitializationAsync()
        {
            ...
            this.CancelLogicalTimeManagerInitialization();
        }
        ...
        private void InitializeAndStartLogicalTimeManager(CancellationToken cancellationToken)
        {
            try
            {
                ...
                using (var tx = this.storeReplica.CreateTransaction())
                {
                    using var enumerator = this.storeReplica.Enumerate(tx, LogicalTimestampKey);
            ...
            {
                ActorTrace.Source.WriteErrorWithId(TraceType, this.traceId, "Logical time manager inittialization failed: {0}", ex);
            }
        }

        private void CancelLogicalTimeManagerInitialization()
        {
            // Stop logical timer if it is running
            if (this.isLogicalTimeManagerInitialized == true)
            {
